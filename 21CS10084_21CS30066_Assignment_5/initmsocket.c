            ...
            if (!MTP_Table[i].free)
            {
                ...
                if (is_tout)
                {
                    ...
                    while (left != (right + 1) % SEND_BUFFSIZE)
                    {
                        ...
                        printf("user %d: Sending data \"%s\" due to TIMEOUT at %ld\n", i, udp_data, MTP_Table[i].swnd.last_active_time[left]);
                ...
                else
                {
                    ...
                    while (left != (right + 1) % SEND_BUFFSIZE)
                    {
                        ...
                        MTP_Table[i].swnd.last_active_time[left] = time(NULL);
                        printf("user %d: Sending data \"%s\" due to NOT SENT EARLLIER at %ld\n", i, udp_data, MTP_Table[i].swnd.last_active_time[left]);
