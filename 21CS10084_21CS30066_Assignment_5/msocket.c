...
int m_recvfrom(int socket_id, char *buffer, int size, int flags, struct sockaddr *dest, int *len)
{
    ...
    for(int i=0; i<RECV_BUFFSIZE; i++)
    {
        ...
        {
            // printf("User last taken : %d\n", MTP_Table[socket_id].rwnd.last_user_taken);
            // printf("*** age *** RECV BUFFER : ");
            // for (int k = 0; k < RECV_BUFFSIZE; k++)
            // {
            //     printf("%d ", MTP_Table[socket_id].recv_buff[k].sequence_no);
            // }
            // printf("\n");
            my_strcpy(buffer, MTP_Table[socket_id].recv_buff[i].data, KB);
            MTP_Table[socket_id].recv_buff[i].sequence_no = -1;
            // printf("*** pore *** RECV BUFFER : ");
            // for (int k = 0; k < RECV_BUFFSIZE; k++)
            // {
            //     printf("%d ", MTP_Table[socket_id].recv_buff[k].sequence_no);
            // }
    ...
    // }
    // if(min_logical(min_seqno, MTP_Table[socket_id].rwnd.last_inorder_received, MAX_SEQ_NO)==min_seqno)
    // {
    //     for (int i = 0; i < RECV_BUFFSIZE; i++)
    //     {
    //         if(MTP_Table[socket_id].recv_buff[i].sequence_no == min_seqno)
